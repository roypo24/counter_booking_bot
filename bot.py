...
# Saturday=5, Sunday=6, Tuesday=1, Wednesday=2
RUNS = {
...
def sleep_until(target: dt.datetime) -> None:
    while True:
        rem = (target - now_local()).total_seconds()
        if rem <= 0:
            return
        time.sleep(min(0.3, rem / 2))


def precise_wait_until(target_perf: float) -> None:
    while True:
        if time.perf_counter() >= target_perf:
            return
...
def locate_all(image_name: str, confidence: float):
    img = screenshot_rgb()
    template = cv2.imread(asset(image_name))
...
def box_for_target_class(target_time: str):
    """
    מוצא את כרטיס חדר הכושר עם השעה הנכונה:
    1) מאתר את כל המופעים של title_gym.png
    2) בודק מעל כל מופע אם באזור הכותרת מופיעה השעה הרצויה
    """
    ...
    for x, y, w, h in titles:
        # אזור זמן מעל הכותרת, באותו כרטיס
        rx1 = max(0, x - 260)
        ry1 = max(0, y - 110)
        ...
        if target_time.replace(":", "") in txt.replace(":", ""):
            # מחזירים box גדול יותר של מרכז הכרטיס כדי ללחוץ עליו
            card_x1 = max(0, x - 300)
            card_y1 = max(0, y - 150)
...
def choose_day(day_key: str) -> None:
    for _ in range(3):
        if click_image_once(DAY_IMAGE[day_key], confidence=0.82):
            time.sleep(0.6)
            return
        time.sleep(0.2)
    raise RuntimeError("Could not click correct day tab")


def go_to_schedule() -> None:
    for _ in range(3):
        if click_image_once("tab_shiurim.png", confidence=0.82):
            time.sleep(0.8)
            return
        time.sleep(0.2)
...
def main():
    ...
    log(f"Next run: {target_dt.isoformat()} | day={day_key} | class={target_time}")
    ...
    # המתנה מדויקת יחסית ל-T-100ms
    delta = (card_click_dt - now_local()).total_seconds()
    if delta > 0.3:
        time.sleep(delta - 0.2)

    target_perf = time.perf_counter() + max(0.0, (card_click_dt - now_local()).total_seconds())
    precise_wait_until(target_perf)

    # לחיצה על הכרטיס 0.1 שניות לפני פתיחת ההרשמה
    pyautogui.click()
    log("Class card clicked")

    # עכשיו לולאה מהירה ללחיצה על הרשמה ברגע שהוא מופיע
    ...
    while time.perf_counter() < end_time:
        ...
        if btn:
            click_center(btn)
            time.sleep(0.03)
            pyautogui.click()  # קליק נוסף קצר, לעזור אם יש delay
            clicked = True
